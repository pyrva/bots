...
class Utilities(commands.Cog):
    ...
    @staticmethod
    def ping_pong(ctx, bot):  # Alias for the Z.ping command
    ...
    @commands.command(
        name='ping', help='measures the latency of the bot', pass_context=True
    )
    ...
    @commands.command(
        aliases=["pip", "pypi"], help='searches for pip package', pass_context=True
    )
        ...
        if not package:
            await ctx.send(
                embed=discord.Embed(
                    ...
                    description="Invalid pacakge name!", colour=discord.Colour.red()
                )
            )
        else:
    ...
    async def github(self, ctx, endpoint):
        try:
            ...
            embed.add_field(
                name="License", value=info["license"]["name"]
                if info["license"] is not None else "None"
            )
